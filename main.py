...
# Variável global para armazenar o número de cliques no botão "Não"
num_nao_clicados = 0
...
def on_draw():
    ...
    if not hidden:
        message.draw()
        # Adiciona o texto com o número de cliques na tela
        num_clicks_label = pyglet.text.Label(f"Botão 'Não' clicado {num_nao_clicados} vezes",
                                            font_name='Arial',
                                            font_size=16,
                                            x=40, y=80)
        num_clicks_label.draw()
